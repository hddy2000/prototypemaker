...
// 器官恢复系统
const ORGAN_REGEN_INTERVAL = 8000;  // 每8秒恢复被摧毁的器官
const ORGAN_REGEN_BATCH = 2;        // 每次恢复最多2个器官
...
export class PinballScene extends Phaser.Scene {
  ...
  private preventStall() {
    ...
    if (speed < MIN_BALL_SPEED) {
      this.lowSpeedFrames++;
      // 连续 45 帧（约0.75秒）低速即补能
      if (this.lowSpeedFrames > 45) {
    ...
    // 连续90帧（约1.5秒）在小范围内停留 → 强制弹开
    // 使用更大范围的角度（含水平方向）帮助球逃离bumper群
    if (this.stallFrames > 90) {
      // 随机选择向上偏左或偏右，加大水平分量逃离bumper群
      const goLeft = Math.random() < 0.5;
      const angle = goLeft
        ? Phaser.Math.FloatBetween(-Math.PI * 0.85, -Math.PI * 0.55)
        : Phaser.Math.FloatBetween(-Math.PI * 0.45, -Math.PI * 0.15);
      const boost = 500;
      this.ballBody.setVelocity(
        Math.cos(angle) * boost,
        Math.sin(angle) * boost
  ...
  private updateOrganRegen(delta: number) {
    ...
    // 每次恢复最多 ORGAN_REGEN_BATCH 个被摧毁的器官
    let regenCount = 0;
    for (const regenTarget of this.organs) {
      if (regenCount >= ORGAN_REGEN_BATCH) break;
      if (!regenTarget.destroyed) continue;
