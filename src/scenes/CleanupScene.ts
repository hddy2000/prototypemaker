...
const AUTO_CLEAN_DEPOSITS = 4;    // 每投喂4次触发自动清洁
const AUTO_CLEAN_AMOUNT = 10;     // 自动清洁去除全部车厢10%残秽
...
export class CleanupScene extends Phaser.Scene {
  ...
  private depositCount = 0;      // 已投喂次数（每4次触发自动清洁）
  ...
  private autoCleanPollution() {
    ...
    this.cameras.main.flash(400, 100, 255, 200);
    this.showMessage('车厢自动清洁启动！全部车厢残秽 -10%', 2500);
  }
  ...
  private createUI() {
    ...
    this.add.text(400, 575, '方向键/WASD移动 • 空格吸取残秽 • E交互(拾取/封锁/投喂/躲藏) • 被怪物碰到即死 • 小道黄光时停下！', {
